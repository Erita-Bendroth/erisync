...
export function WeeklyDutyCoverageManager({ open, onOpenChange }: WeeklyDutyCoverageManagerProps) {
  ...
  const loadTemplate = async (templateId: string) => {
    ...
    if (!error && data) {
      ...
      // Load table layout if it exists in the template
  ...
  const saveTemplate = async (includeLayout: boolean = false) => {
    ...
    const templateData: any = {
      ...
      created_by: user?.id,
      ...(includeLayout && customRegions.length > 0 && {
        table_layout: {
    ...
    if (error) {
      ...
      const message = includeLayout ? "Template and layout saved successfully" : "Template saved successfully";
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex gap-2">
                  <Button onClick={() => saveTemplate(false)} disabled={loading}>
