...
DROP POLICY IF EXISTS "authenticated_users_view_profiles_secure" ON public.profiles;

-- Create single secure SELECT policy for profiles requiring authentication
CREATE POLICY "auth_profiles_view_v2"
ON public.profiles
FOR SELECT
TO authenticated
USING (
...
DROP POLICY IF EXISTS "authenticated_users_own_oauth_tokens_secure" ON public.user_oauth_tokens;

-- Create single secure SELECT policy for OAuth tokens requiring strict ownership
CREATE POLICY "auth_oauth_tokens_view_v2"
...
DROP POLICY IF EXISTS "authenticated_users_view_vacation_requests_secure" ON public.vacation_requests;

-- Create single secure SELECT policy for vacation requests requiring proper authorization
CREATE POLICY "auth_vacation_requests_view_v2"
