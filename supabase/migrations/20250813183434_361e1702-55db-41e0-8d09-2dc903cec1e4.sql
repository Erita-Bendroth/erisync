-- Fix the unique constraint issue in the bulk schedule generation
-- The current function uses (user_id, date, team_id) but we have a constraint on (user_id, date)
-- We need to update the ON CONFLICT clause to match the correct constraint
...
BEGIN
    ...
    WHILE current_date_var <= _end_date LOOP
        ...
        -- Only create entries for Monday (1) through Friday (5)
        IF day_of_week BETWEEN 1 AND 5 THEN
            ...
            IF NOT EXISTS (
                ...
                -- Use the correct constraint name: schedule_entries_user_date_team_unique
