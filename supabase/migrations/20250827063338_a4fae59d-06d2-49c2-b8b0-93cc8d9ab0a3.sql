-- Add region support to profiles table for German state/region-specific holidays
ALTER TABLE public.profiles 
ADD COLUMN region_code text;

-- Add region support to holidays table
ALTER TABLE public.holidays 
ADD COLUMN region_code text;

-- Create index for better performance when filtering by region
CREATE INDEX idx_holidays_region_code ON public.holidays(region_code);
CREATE INDEX idx_profiles_region_code ON public.profiles(region_code);

-- Update the create_default_schedule_with_holidays function to include region support
CREATE OR REPLACE FUNCTION public.create_default_schedule_with_holidays(_user_id uuid, _team_id uuid, _start_date date, _end_date date, _created_by uuid, _country_code text DEFAULT NULL::text, _region_code text DEFAULT NULL::text)
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
        ...
        IF day_of_week BETWEEN 1 AND 5 THEN
            ...
            IF NOT EXISTS (
                ...
                AND (
                    ...
                    region_code IS NULL
                    -- Regional holidays matching user's region
                    OR (region_code = user_region AND user_region IS NOT NULL)
                )
